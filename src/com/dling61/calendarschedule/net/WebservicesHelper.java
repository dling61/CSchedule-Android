...
//										TabActivity.class);
			...
			public void onSuccess(JSONObject response) {
				...
				try {
					...
					for (int i = 0; i < service_count; i++) {
						...
						// int activityid =
						// ownid*10000+service.getInt("serviceid");
			...
			public void onSuccess(JSONObject response) {
				...
				try {
					...
					for (int i = 0; i < scheudule_count; i++) {
						...
						// cv.put(ScheduleTable.schedule_ID,
						// (Schedule.getInt("creatorid")*10000+Schedule.getInt("scheduleid")));
						Log.i("scheduleid",
								"= " + Schedule.getInt("scheduleid"));
	...
	{
		String ParticipantUrl = BaseUrl.BASEURL + "members" + "?" + BaseUrl.URL_POST_FIX;
		Log.i ("url is :" , ParticipantUrl);
		final SharedReference ref=new SharedReference();
		int currentOwnerID = ref.getCurrentOwnerId(mContext);
		RequestParams params = new RequestParams();
		params.put("ownerid", String.valueOf(currentOwnerID));
		params.put("lastupdatetime",ref.getLastestParticipantLastModifiedTime(mContext));
			...
			public void onSuccess(JSONObject response) {
	            ...
	            try {
					...
					for (int i = 0; i < participant_count; i++)
					{
						...
						int ownerConstant = ownerid * 10000;
						
						//Owner is included in JSON Response "members"
						//Should not appear in Participants
						
						ContentValues cv = new ContentValues();
						cv.put(ParticipantTable.last_Modified, Participant.getString("lastmodified"));
						cv.put(ParticipantTable.participant_Name, Participant.getString("membername"));
						//cv.put(ParticipantTable.own_ID, Participant.getInt("creatorid"));
						cv.put(ParticipantTable.own_ID, ownerid);
						Log.i("getParticipantsFromWeb own_ID ", ownerid+"");
						int participant_ID = (Participant.getInt("memberid")+1);
						cv.put(ParticipantTable.participant_ID, participant_ID);
						Log.i("getParticipantsFromWeb participant_ID ", participant_ID+"");
						String participant_Mobile = Participant.getString("mobilenumber");
						cv.put(ParticipantTable.participant_Mobile, participant_Mobile);
						Log.i("getParticipantsFromWeb participant_Mobile ", participant_Mobile);
						String participant_Email = Participant.getString("memberemail");
						Log.i("getParticipantsFromWeb participant_Email ", participant_Email);
						...
						Log.i("getParticipantsFromWeb participantID ",participantID+"");
						if (dbHelper.IsParticipantExisted(participantID) == false )
						{
							if(dbHelper.insertParticipant(cv))
								Log.i("database", "insert participant " + Participant.getString("membername") + " successfully!");
						}
						else
						{
							if(dbHelper.updateParticipant(participantID, cv))
								Log.i("database", "update participant " + Participant.getString("membername") + " successfully!");
						}
					}
					
					ref.setLastestParticipantLastModifiedTime(mContext, MyDate.transformLocalDateTimeToUTCFormat(MyDate.getCurrentDateTime()));
	...
	{
		String ActivityUrl = BaseUrl.BASEURL + "services" + "?" + BaseUrl.URL_POST_FIX;
		...
		try {
			...
			client.post(null,ActivityUrl,entity,"application/json", new JsonHttpResponseHandler() {
	            public void onSuccess(JSONObject response) {
	            	Log.i("successful response", response.toString());
	            	try {
	            		ContentValues cv  = new ContentValues();
	            		String last_modified = response.getString("lastmodified");
	            		cv.put(ActivityTable.last_ModifiedTime, last_modified);
	...
	public void addSchedule(Schedule schedule, List<Integer> pins)
	{
		...
		String ScheduleUrl = BaseUrl.BASEURL + "schedules"+ "?" + BaseUrl.URL_POST_FIX;;
		...
		try {
			...
			StringEntity entity = new StringEntity(params.toString());
			client.post(null,ScheduleUrl,entity,"application/json", new JsonHttpResponseHandler() {
	            public void onSuccess(JSONObject response) {
	            	Log.i("successful response", response.toString());
	            	try {
	            		ContentValues cv  = new ContentValues();
	            		String last_modified = response.getString("lastmodified");
	            		cv.put(ScheduleTable.last_Modified, last_modified);
	            		cv.put(ScheduleTable.is_Synchronized, 1);	
	...
	{
		//String ScheduleUrl = BASEURL + "schedules/" + schedule.getScheduleID();
		String ScheduleUrl = BaseUrl.BASEURL + "schedules/" + schedule.getSchedule_ID()+ "?" + BaseUrl.URL_POST_FIX;
		final int id = schedule.getSchedule_ID();
		try {
			JSONObject scheduleParams = new JSONObject();
			scheduleParams.put("desp", schedule.getDesp());
			...
			Log.i("add activity", params.toString());
			StringEntity entity = new StringEntity(params.toString());
			client.put(null,ScheduleUrl,entity,"application/json", new JsonHttpResponseHandler() {
	            public void onSuccess(JSONObject response) {
	            	Log.i("successful response", response.toString());
	            	try {
	            		ContentValues cv  = new ContentValues();
	...
	{
		List<MyActivity> unsyncedActivities = dbHelper.getUnsyncedNewActivities();
	...
	{
		List<MyActivity> unsyncedActivities = dbHelper.getUnsyncedEditedActivities();
		for (int i = 0; i < unsyncedActivities.size() ; i++)
		{
			MyActivity ma = unsyncedActivities.get(i);
			updateActivity(ma);
	...
	{
		List<Participant> unsyncedParticipants = dbHelper.getUnsyncedNewParticipants();
	...
	{
		List<Participant> unsyncedParticipants = dbHelper.getUnsyncedEditedParticipants();
		for (int i = 0; i < unsyncedParticipants.size() ; i++)
		{
			Participant p = unsyncedParticipants.get(i);
	...
	public void uploadRecentNewSchedulesToWeb()
	{
		...
		for (int i = 0; i < unsyncedSchedules.size() ; i++)
		{
			...
			List<Integer> members = dbHelper.getParticipantsForSchedule(s.getSchedule_ID());
	...
	{
		List<Schedule> unsyncedSchedules = dbHelper.getUnsyncedEditedSchedules();
		for (int i = 0; i < unsyncedSchedules.size() ; i++)
		{
			Schedule s = unsyncedSchedules.get(i);
			List<Integer> members = dbHelper.getParticipantsForSchedule(s.getSchedule_ID());
	...
	//
	public void getSharedmembersForActivity (final int activity_id, final List<Sharedmember> sharedmembers)
	{
		String SharedmembersUrl = BaseUrl.BASEURL + "services/" + activity_id + "/sharedmembers" + "?" + BaseUrl.URL_POST_FIX;
		Log.i ("url is :" , SharedmembersUrl);
		int currentOwnerID = mContext.getSharedPreferences("MyPreferences", 0).getInt("currentownerid", 0);
		RequestParams params = new RequestParams();
		params.put("ownerid", String.valueOf(currentOwnerID));
		params.put("lastupdatetime", "");
		client.get(SharedmembersUrl, params,new JsonHttpResponseHandler() 
		{
			public void onSuccess(JSONObject response) {
	            Log.i("successful response", response.toString());
	            try {
					JSONArray JSharedmembers = response.getJSONArray("sharedmembers");
					int sm_count = JSharedmembers.length();
					for (int i = 0; i < sm_count; i++)
					{
						JSONObject JSharedmember = JSharedmembers.getJSONObject(i);
						int sm_id = JSharedmember.getInt("memberid");
						String sm_email = JSharedmember.getString("memberemail");
						String sm_number = JSharedmember.getString("mobilenumber");
						String sm_name = JSharedmember.getString("membername");
						int sm_role = JSharedmember.getInt("sharedrole");
						String sm_lastmdf = JSharedmember.getString("lastmodified");
						Sharedmember newsm = new Sharedmember (sm_id,sm_name,sm_email,sm_number,sm_role,activity_id);
						...
						//cv.put(SharedMemberTable.last_modified, JSharedmember.getString("lastmodified"));
					...
					SharedPreferences sp =  mContext.getSharedPreferences("MyPreferences", 0);
					Editor editor = sp.edit();
					editor.putString("lastparticipantmodified", MyDate.transformLocalDateTimeToUTCFormat(MyDate.getCurrentDateTime()));
					editor.commit();
					
					Intent intent = new Intent();
					intent.setAction("cschedule.sharedmembersready");
					mContext.sendBroadcast(intent);
	...
	public void postSharedmemberToActivity (int memberid, int role, int activityid)
	{
		String sharedmemberUrl = BaseUrl.BASEURL + "services/" + activityid + "/" + "sharedmembers" + "?" + BaseUrl.URL_POST_FIX;
		try {
			JSONObject sharedmemberParams = new JSONObject();
			sharedmemberParams.put("ownerid", new SharedReference().getCurrentOwnerId(mContext));
...
//			Log.i("add participant", sharedmemberParams.toString());
			StringEntity entity = new StringEntity(sharedmemberParams.toString());
			client.post(null,sharedmemberUrl,entity,"application/json", new JsonHttpResponseHandler() {
	            public void onSuccess(JSONObject response) {
	            	Log.i("successful response", response.toString());
	...
	public void deleteSharedmemberOfActivity (int memberid, int activityid)
	{
		String sharedmemberUrl = BaseUrl.BASEURL + "services/" + activityid + "/" + "sharedmembers/" + memberid + "?" + BaseUrl.URL_POST_FIX;
		client.delete(sharedmemberUrl, new JsonHttpResponseHandler() {
	            public void onSuccess(JSONObject response) {
	            	Log.i("successfully delete", response.toString());
	            }
	            public void onFailure(Throwable e, String response) {
	                // Response failed :
	            	Log.i("failure response", response);
	...
	public void alterSharedmemberToActivity (int memberid, int role, int activityid)
	{
		String sharedmemberUrl = BaseUrl.BASEURL + "services/" + activityid + "/" + "sharedmembers/" + memberid + "?" + BaseUrl.URL_POST_FIX;
		try {			
			JSONObject params = new JSONObject();
			params.put("ownerid", new SharedReference().getCurrentOwnerId(mContext));
			params.put("sharedrole", role);
			
//			client.addHeader("Content-type", "application/json");
			Log.i("update participant", params.toString());
			StringEntity entity = new StringEntity(params.toString());
			client.put(null,sharedmemberUrl,entity,"application/json", new JsonHttpResponseHandler() {
	            public void onSuccess(JSONObject response) {
	            	Log.i("successful response", response.toString());
	...
	public void addParticipant(Participant participant)
	{
		...
		try {
			...
			StringEntity entity = new StringEntity(params.toString());
			client.post(null,ParticipantUrl,entity,"application/json", new JsonHttpResponseHandler() {
	            public void onSuccess(JSONObject response) {
	            	Log.i("successful response", response.toString());
	            	try {
	            		ContentValues cv  = new ContentValues();
	            		String last_modified = response.getString("lastmodified");
	            		cv.put(ParticipantTable.last_Modified, last_modified);
	            		cv.put(ParticipantTable.is_Sychronized, 1);	
	...
	{
		String ParticipantUrl = BaseUrl.BASEURL + "members/" + participant.getID() + "?" + BaseUrl.URL_POST_FIX;
		...
		try {
			...
			Log.i("update participant", params.toString());
			StringEntity entity = new StringEntity(params.toString());
			client.put(null,ParticipantUrl,entity,"application/json", new JsonHttpResponseHandler() {
	            public void onSuccess(JSONObject response) {
	            	Log.i("successful response", response.toString());
	            	try {
	            		ContentValues cv  = new ContentValues();
	...
	public void deleteParticipant(Participant participant)
	{
		String ParticipantUrl = BaseUrl.BASEURL + "members/" + participant.getID() + "?" + BaseUrl.URL_POST_FIX;
		final int id = participant.getID();
		client.delete(ParticipantUrl, new JsonHttpResponseHandler() {
	            public void onSuccess(JSONObject response) {
	            	try {
						if (response.getString("lastmodified") != null)
						{
	...
	{
		String ActivityUrl = BaseUrl.BASEURL + "services/" + activity.getActivity_ID() + "?" + BaseUrl.URL_POST_FIX;
		final int id = activity.getActivity_ID();
		try {
			JSONObject activityParams = new JSONObject();
			activityParams.put("alert", activity.getAlert());
			...
			StringEntity entity = new StringEntity(params.toString());
			client.put(null,ActivityUrl,entity,"application/json", new JsonHttpResponseHandler() {
	            public void onSuccess(JSONObject response) {
	            	Log.i("successful response", response.toString());
	            	try {
	            		ContentValues cv  = new ContentValues();
		...
		client.delete(ActivityUrl, new JsonHttpResponseHandler() {
	            public void onSuccess(JSONObject response) {
	            	try {
						if (response.getString("lastmodified") != null)
						{
							List<Schedule> sbelongtoa = dbHelper.getSchedulesBelongtoActivity(id);
